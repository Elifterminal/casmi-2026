#!/usr/bin/env python3
"""Port check 4: predict2 with the E37 tail fix must reproduce E37's by_score arm.

Port check 3 held the shipped module to E25's sqrt-p arm at 0.368176. E37 then showed that the
spectral-only tail was being ordered alphabetically by InChIKey and fixed it, moving the same
configuration to 0.371845. This is the same check against the new target: each twin-safe eval
split is dressed as a test set, run through predict2 -- the same call the notebook makes -- and
the pooled weighted MRR must equal E37's by_score to 1e-9. The ranker weights are unchanged
(E25's), because the fix touches ordering only, never the learned model.
"""
...
def main():
    t0 = time.time()
    e25 = json.load(open(os.path.join(WORK, "results", "E25_ground.json")))
    arm = e25["arms"]["sqrt-p ground"]
    e37 = json.load(open(os.path.join(WORK, "results", "E37_tail.json")))
    weights = arm["weights"]
    want = dict(arm, weighted=e37["production (NP, in COCONUT)"]["by_score"])
    ...
    for split in EVAL:
        ...
        sindex = cp.SpectralIndex(keys, peaks, ref_rows, sqrt_p=True)
        ...
        pred = cp.predict2(mols, sindex, midx, smi_of, weights, peaks, pmz, procs=7, generate=False,
    ...
    print(f"{'E37':10s} {'':>9} {'':>9} {'':>9} {want['weighted']:>10.4f}   (by_score arm)")
    ok = abs(gotw - want["weighted"]) < 1e-9
    print("\nRESULT:", "PORT EQUIVALENT to E37 by_score" if ok else f"PORT DIFFERS ({gotw-want['weighted']:+.6f})")
