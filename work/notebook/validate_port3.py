#!/usr/bin/env python3
"""Port check for the sqrt-p ground: predict2 (generation OFF) must reproduce E25's sqrt-p arm.
...
def main():
    ...
    print(f"{'E25':10s} {want['C1']:>9.4f} {want['C2']:>9.4f} {want['C3']:>9.4f} {want['weighted']:>10.4f}")
    ok = abs(gotw - want["weighted"]) < 1e-9
    print("\nRESULT:", "PORT EQUIVALENT to E25" if ok else f"PORT DIFFERS ({gotw-want['weighted']:+.6f})")
