#!/usr/bin/env python3
"""E24 — is a molecule's collision-energy series worth aligning, or is pooling it fine?
...
PREDICTIONS (before the run, 2026-09-21):
  P1  pooled_fr is within 0.02 of pooled_cos on median best Tanimoto -- the ground alone is
      second-order, the convention we inherited is not what is costing us.
  P2  aligned_fr beats pooled_cos by >= 0.05 median best Tanimoto. This is the real claim.
  P3  The gain concentrates where it should: queries whose relatives have >= 3 energies.

  RESULT (2026-09-21, 756 Class 2/3 queries) -- BOTH MAIN PREDICTIONS WRONG, IN OPPOSITE
...
def pick_frames(rows, ce, peaks, cap=MAX_FRAMES):
...
def main():
        ...
        for n, (k, rows) in enumerate(qrows.items(), 1):
            ...
            for c in cand:
                ...
                if c not in cache_pool:
                    ...
                    cache_series[c] = pick_frames(rows_c, ce, peaks)
                pf, sf = cache_pool[c], cache_series[c]
                if pf is None or not sf: continue
    ...
    L = [f"E24 — pooled vs energy-aligned relative search, {len(per['pooled_cos'])} Class 2/3 queries", "",
    ...
    print("\n".join(L))
    open(f"{RESULTS}/E24_energy_align_2026-09-21.txt", "w").write("\n".join(L) + "\n")
    json.dump(out, open(f"{RESULTS}/E24_energy_align.json", "w"), indent=2)
