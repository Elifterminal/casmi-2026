...
def main():
    ...
    ALPHAS = [0.0, 0.5, 1.0, 2.0, 4.0]   # E10: rank = frag_raw + alpha*spec_raw
    rr = {rule:defaultdict(list) for rule in RULES}
    for a in ALPHAS: rr[f"fragplus{a}"] = defaultdict(list)
    ...
    for n,(k,rows) in enumerate(qrows.items(),1):
        ...
        if not allc:
            for rule in rr: rr[rule][cls].append(0.0)
            continue
        smax=max(spec.values()) if spec else 1.0; fmax=max(frag.values()) if frag else 1.0
        smax=smax or 1.0; fmax=fmax or 1.0
        nfeats={c:(spec.get(c,0.0)/smax, frag.get(c,0.0)/fmax) for c in allc}   # normalised
        rfeats={c:(spec.get(c,0.0),      frag.get(c,0.0))      for c in allc}   # RAW (E10)
        def score_list(ranked):
            for i,c in enumerate(ranked[:TOPN],1):
                if ckey(c)==truth: return 1.0/i
            return 0.0
        # normalised rules (E08/E09 baselines), deterministic tie-break on key
        for rule,fn in RULES.items():
            rr[rule][cls].append(score_list(sorted(allc, key=lambda c:(-fn(*nfeats[c]), c))))
        # E10: RAW scores. frag is the universal signal (both classes have it);
        # spec is the C1-specific bonus. Sweep how much spec to add back.
        for a in ALPHAS:
            rr[f"fragplus{a}"][cls].append(
                score_list(sorted(allc, key=lambda c:(-(rfeats[c][1] + a*rfeats[c][0]), c))))
    ...
    out={}
    for rule in list(RULES)+[f"fragplus{a}" for a in ALPHAS]:
